...
| `test-elixir-component.sh` | the `elixir` component's catalog shape + `run.sh` wiring, and its install contract: versions pinned (no floating `latest`) with the Elixir `-otp-NN` suffix matching the pinned OTP major; mise/mix always `runuser`-wrapped as `$AGENT_USER`; shims symlinked onto `/usr/local/bin` (dispatched-job PATH) with no dangling `rebar3`/`mix.ps1`; Hex + rebar bootstrapped; the 5 apt packages; agent-owned Dialyzer PLT dir; the description's docker-pairing + disk guidance |
