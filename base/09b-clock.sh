...
if ! [[ "$SB_TZ" =~ $SB_TZ_RE ]] || [ ! -f "${SB_ZONEINFO}/${SB_TZ}" ]; then
  ...
  log "time zone already ${SB_TZ}"
         ...
         || ln -sfn "${SB_ZONEINFO}/${SB_TZ}" "$SB_LOCALTIME" 2>/dev/null; } \
...
# strftime pattern). Validated tmp + mv, as in 14, so a jq failure never
# corrupts settings.json.
...
elif jq '.timeFormat = "24-hour"' "$CLAUDE_SETTINGS" > "${CLAUDE_SETTINGS}.tmp" 2>/dev/null; then
